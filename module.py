...
def get_rtsp(rtsp_url,num,st=None):
    ...
    global array1,array2,array3,array4,rtsp1,rtsp2,rtsp3,rtsp4
...
def get_image_threading():
    g = Thread(target=get_image_from_array)
    g.daemon = True
    g.start()
...
def get_image_from_array():
    ...
    while True:
        if len(array1) > 0:
            print(f'check length a1: {len(array1)}')
            image1 = array1[0]
            image_processing(image1, rtsp1)
            array1.pop(0)
        if len(array2) > 0:
            print(f'check length a2: {len(array2)}')
            image2 = array2[0]
            image_processing(image2, rtsp2)
            array2.pop(0)
        if len(array3) > 0:
            print(f'check length a3: {len(array3)}')
            image3 = array3[0]
            image_processing(image3, rtsp3)
            array3.pop(0)
        if len(array4) > 0:
            print(f'check length a4: {len(array4)}')
            image4 = array4[0]
            image_processing(image4, rtsp4)
            array4.pop(0)

def image_processing(frame,num):
    save_img = read_nvr(3)
    polygon_employ, polygon_customer = read_polygon_value(num)
    if polygon_employ != False:
        ...
        output_flask_process, output_flask_process_gender_age = request_post_onprocess(num, frame, Date, Time,
                                                                                       file_name, polygon_customer,
